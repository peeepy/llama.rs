...
impl ModelTokenizer {
    pub fn new<P: AsRef<Path>>(model_dir: P) -> Result<Self, Box<dyn Error + Send + Sync>> {
        let config = ModelTokenizerConfig::from_directory(model_dir)
            .map_err(|e| Box::<dyn Error + Send + Sync>::from(e))?;
        let tokenizer = Tokenizer::from_file(&config.tokenizer_json).map_err(|e| {
    ...
    fn load_special_tokens(&mut self) -> Result<(), Box<dyn Error + Send + Sync>> {
        if let Some(special_tokens_path) = &self.config.special_tokens_map_json {
            let content = fs::read_to_string(special_tokens_path).map_err(|e| {
                Box::<dyn Error + Send + Sync>::from(format!(
                    "Failed to read special_tokens_map.json: {:?}",
                    e
                ))
            })?;
            let special_tokens: Value = serde_json::from_str(&content).map_err(|e| {
                Box::<dyn Error + Send + Sync>::from(format!(
                    "Failed to parse special_tokens_map.json: {:?}",
                    e
                ))
            })?;
    ...
    pub fn encode(&self, text: &str) -> Result<Vec<u32>, Box<dyn Error + Send + Sync>> {
        let encoding = self.tokenizer.encode(text, false).map_err(|e| {
            Box::<dyn Error + Send + Sync>::from(format!("Failed to encode text: {:?}", e))
        })?;
    ...
    pub fn decode(&self, token_ids: &[u32]) -> Result<String, Box<dyn Error + Send + Sync>> {
        self.tokenizer.decode(token_ids, false).map_err(|e| {
            Box::<dyn Error + Send + Sync>::from(format!("Failed to decode tokens: {:?}", e))
        })
    }
    ...
    pub fn get_generation_config(&self) -> Result<Value, Box<dyn Error + Send + Sync>> {
        ...
        if let Some(content) = content {
            let value: Value = serde_json::from_str(&content).map_err(|e| {
                Box::<dyn Error + Send + Sync>::from(format!(
                    "Failed to parse generation_config.json: {:?}",
                    e
                ))
            })?;
            Ok(value)
            ...
            Err(Box::<dyn Error + Send + Sync>::from(
                "No generation_config.json found",
            ))
        }
    }
}
