...
impl QuantizedTensor {
    ...
    //             let q1 = ((pair.get(0) / scale).round() + 8.0).clamp(0.0, 15.0);
    //             let q2 = if pair.size()[0] > 1 {
    //                 ((pair.get(1) / scale).round() + 8.0).clamp(0.0, 15.0)
    //             } else {
    //                 Tensor::of_slice(&[0.0]).unwrap() // If missing second value, use 0
    //             };

    //             let packed_q4 = (q1.int64_value(&[]) as u8) | ((q2.int64_value(&[]) as u8) << 4);
    ...
    //         QuantType::None => self.data.clone(),  // Return the original data directly
    //         QuantType::Q8_0 => self.dequantize_q8_0().unwrap(),
    //         QuantType::Q4_0 => self.dequantize_q4_0(),
    //     }
    // }

    // // Dequantize Q8_0
    // fn dequantize_q8_0(&self) -> Result<Tensor, TchError> {
    //     let mut dequantized = vec![];
    //     let data = self.data.to_kind(tch::Kind::Int8);

    //     for (i, &q) in data.iter::<i8>()?.enumerate() {
    ...
    //     for (i, &packed) in data.iter::<i8>().unwrap().enumerate() {
    ...
    //     Tensor::of_slice(&dequantized).unwrap()
    // }
}
...
//         QuantState::Quantized(data) => Tensor::from_slice(data),
//         QuantState::Unquantized(data) => Tensor::from_slice(data),
//     };
...
//     Ok(Self {
//         data: QuantState::Quantized(data),
//         scales,
//         quant_type,
//         group_size,
//         original_shape,
